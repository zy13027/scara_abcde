...
**Last updated:** 2026-05-21 (motion/process decouple committed — OB30 axis layer + FB_AxisCtrl V2.1 on LKinCtrl; LKinCtrl library operator-approved, B.26 closed)
**Last action:** Committed `502edd6` — axis/process decouple onto an OB30 axis layer (`FB_AxisCtrl` V2.1 on LKinCtrl, new `FB_Init` homing, `FB_AutoCtrl_5Pts` CASE skeleton, 3 new iDBs; 10 files, +565/−486). LKinCtrl library use **operator-approved** this session → B.26 closed ✅ (recorded in B.26 + ledger, not in the commit message). DB-shape change → memory reset owed; `FB_AutoCtrl_Palletizing` next cluster in flight.
