...
**Last updated:** 2026-05-21 (single-owner git cycle running — §4 reconciliation done; layered-refactor phases R1–R3 + project README committed and pushed; `origin/main` → `d7c7765`)
**Last action:** Ran the held §4 git reconciliation (`33fe91d` pushed — single-owner rule now live on `origin`), then committed + pushed six commits this cycle as sole git operator on scara-PLC "phase ready" signals: `e9affe6` (`UDT_typeKinCtrl` structify) · `2e3a9de`+`787a9b1` (`GDB_Control` retired → structured `GDB_AxisCtrl`, dead DB deleted) · `5458010` (new `FB_Init` homing FB) · `d7c7765` (project README). `origin/main` in sync, working tree clean. Refactor R1–R3 done; R4–R6 (MC_* out of 500, CASE step model, Pause step) remain.
...
| B.25 | R1–R6 layered code refactor execution | 🚧 R1/R2 done — `GDB_Control` retired, consolidated into structured `GDB_AxisCtrl.LKinCtrl.*` (`e9affe6`+`2e3a9de`+`787a9b1`; dead `GDB_Control.xml` deleted). R3 done — new `FB_Init` homing FB (`5458010`). **Open `[NEEDS_scaraPLC]`:** R4 (MC_*/MovePath out of 500), R5 (FB_AutoCtrl_5Pts+ABCDE→standard CASE), R6 (Pause step). Debt: ~30 stale `GDB_Control` comments (sweep owed). |
| B.26 | LKinCtrl library usage decision | ⏸️ `[NEEDS_OPERATOR]` — 郑磊 (郑老板) must rule whether the L Kinematics Control library is permitted (conflicts with project「不用任何的库」rule). **Escalating:** the R1–R3 refactor has built real LKinCtrl coupling anyway — `GDB_AxisCtrl.LKinCtrl.*` across Startup/HMI-mirror/ManualCtrl/FB_AxisCtrl, and `FB_Init` on `LKinCtrl_MC_GroupHome`. A "no" now means rework; cost rises every phase. Operator owns getting the decision. |
